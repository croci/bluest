...
plt.rc('xtick', labelsize=fntsz)     
plt.rc('ytick', labelsize=fntsz)
...
fig = newfig(0)
plt.subplot(2,2,1)
#v0 = np.mean(np.array([item for item in bud_list[-1]['v_list'][0]]))
#cost0 = 56299.4444444444444
v0 = 0.00077685
...
for i in range(len(v_list)):
    ...
    efficiency_list.append(np.log10(BUDGET*v0**2/(BUDGET*np.array(v_list[i])**2)))

v1 = plt.violinplot(efficiency_list, positions=1*np.arange(1, len(v_list)+1))
plt.ylabel(r"normalized estimator efficiency")
plt.xlabel(r"$n_{\text{HF}}$", labelpad=-20)
ax = fig.gca()
ax.yaxis.set_major_locator(MaxNLocator(integer=True))

#plt.title("Approach a)", fontsize=ttlfntsz)
plt.title(r"Approach a), $n_{\text{HF}}$ pilot samples of all models", fontsize=ttlfntsz)
...
for d in range(dmax):
    ...
    v1 = plt.violinplot(efficiency_list, positions=1*np.arange(1, len(v_list)+1))
    if d%2 == 1: plt.ylabel(r"normalized estimator efficiency")
    plt.xlabel(r"$n_{\text{HF}}$", labelpad=-20)
    ax = fig.gca()
    ax.yaxis.set_major_locator(MaxNLocator(integer=True))
    if d > 0: ax.set_yticks(np.array([-1,0]))

    plt.title(r"Approach b), extrapolating covariances with $\bar{d}=%d$ models" % (d+1), fontsize=ttlfntsz)
